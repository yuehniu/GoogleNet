...
class Flowers102DataLoader(DataLoader):
    ...
    def load_data(self):
        ...
        mat = scipy.io.loadmat(os.path.join(self.config.data_dir, 'imagelabels.mat'))
        names = os.listdir(os.path.join(self.config.data_dir, image_dir))
        names.sort()
        image_list = [os.path.join(self.config.data_dir, image_dir, x) for x in names]
